...
1. Author writes/modifies code on a feature branch
        ↓
2. Author runs local self-tests (lint, syntax, unit, integration)
        ↓
...
- `weighted_total` ≥ 85 (weighted sum of all 10 dimensions)
...
- Network downloads without HTTPS or without integrity verification
- Empty `catch {}` blocks
...
PowerShell scripts that install Node.js, MongoDB, MeshCentral as services, open firewall ports, and create SMB shares can easily produce an insecure cluster controller or a broken host. A second independent pair of eyes on every change catches what the author missed. The rubric is intentionally strict — the cost of a "lenient pass" that exposes the cluster brain is much higher than the cost of one extra review cycle.
