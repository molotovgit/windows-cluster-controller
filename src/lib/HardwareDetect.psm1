<#
.SYNOPSIS
    Hardware and OS-edition detection for the windows-cluster-controller
    setup script. Every function tries multiple strategies and returns the
    first one that yields a useful answer.
...
# admin rights or actual hardware. Gated by CLUSTERCTRL_ALLOW_TEST_SEAMS.
...
function Confirm-TestSeamAllowed {
    if (-not $env:CLUSTERCTRL_ALLOW_TEST_SEAMS) {
        throw "Test seams are disabled in production. Set `$env:CLUSTERCTRL_ALLOW_TEST_SEAMS=1 to enable Set-HardwareDetector / Reset-HardwareDetector."
    }
}
...
# After setting $env:CLUSTERCTRL_ALLOW_TEST_SEAMS=1.
