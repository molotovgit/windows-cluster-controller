<#
.SYNOPSIS
    Persistent setup-run state for the windows-cluster-controller orchestrator.
...
.DESCRIPTION
    ...
    Registry layout (under HKLM:\Software\ClusterController\ by default):
    ...
    Tests redirect the base key via $env:CLUSTERCTRL_REG_BASE, which must
...
.NOTES
    Scheduled task name defaults to 'ClusterControllerResume'. The task runs at
...
function Get-DefaultRegBase {
    if ($env:CLUSTERCTRL_REG_BASE) { return $env:CLUSTERCTRL_REG_BASE }
    return 'HKLM:\Software\ClusterController'
}
...
function Confirm-RegBase {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        try {
            ...
            throw "Cluster-controller state cannot create registry key '$Path' ($($_.Exception.Message)). Run as Administrator or override with `$env:CLUSTERCTRL_REG_BASE."
        }
    }
}
...
function Save-StageMarker {
    ...
    .PARAMETER RegBase
        ...
        $env:CLUSTERCTRL_REG_BASE or HKLM:\Software\ClusterController.
    #>
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
        'PSUseShouldProcessForStateChangingFunctions', '',
        Justification = 'Writes to a dedicated cluster-controller registry key only; no ShouldProcess prompt is appropriate for an unattended setup script.')]
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [ValidateRange(1, 32)]
...
function Clear-StageMarker {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
        'PSUseShouldProcessForStateChangingFunctions', '',
        Justification = 'Operates only on a dedicated cluster-controller registry key value.')]
    [CmdletBinding()]
    param([string]$RegBase)
    if (-not $RegBase) { $RegBase = Get-DefaultRegBase }
...
function Set-ClusterRunStatus {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
        'PSUseShouldProcessForStateChangingFunctions', '',
        Justification = 'Writes to a dedicated cluster-controller registry key only.')]
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
...
function Reset-ClusterRunState {
    <#
    .SYNOPSIS
        Wipe ALL cluster-controller run state from the registry root. Used at the
        very start of a fresh run (orchestrator's -Restart switch) and by
        unit tests. Does NOT touch the scheduled task.
    #>
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
        'PSUseShouldProcessForStateChangingFunctions', '',
        Justification = 'Operates only on the dedicated cluster-controller registry key tree.')]
    [CmdletBinding()]
    param([string]$RegBase)
    if (-not $RegBase) { $RegBase = Get-DefaultRegBase }
...
$script:ResumeTaskName    = 'ClusterControllerResume'
$script:ResumeTaskFolder  = '\ClusterController\'   # dedicated folder to avoid name collisions
...
# Set-ResumeTaskInvoker, which is gated by CLUSTERCTRL_ALLOW_TEST_SEAMS.
...
function Confirm-TestSeamAllowed {
    if (-not $env:CLUSTERCTRL_ALLOW_TEST_SEAMS) {
        throw "Test seams are disabled in production. Set `$env:CLUSTERCTRL_ALLOW_TEST_SEAMS=1 to enable Set-ResumeTaskInvoker / Reset-ResumeTaskInvoker."
    }
}
...
function Set-ResumeTaskInvoker {
    ...
    .SYNOPSIS
        ...
        CimInstance parameter binding. Gated by CLUSTERCTRL_ALLOW_TEST_SEAMS.
...
function Reset-ResumeTaskInvoker {
    <#
    .SYNOPSIS Test-only: restore the real-cmdlet invokers. Gated by CLUSTERCTRL_ALLOW_TEST_SEAMS.
    #>
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
        'PSUseShouldProcessForStateChangingFunctions', '',
...
function Confirm-TaskSchedulerAvailable {
    ...
    if ($env:CLUSTERCTRL_ALLOW_TEST_SEAMS) { return }
...
# AND must set $env:CLUSTERCTRL_ALLOW_TEST_SEAMS=1 before the call.
