...
BeforeAll {
    ...
    $env:CLUSTERCTRL_LOG_DIR   = $script:tmpRoot
    $env:CLUSTERCTRL_STATE_DIR = $script:stateRoot
}
...
Describe 'Initialize-ClusterLog' {
    ...
    It 'writes an "opened" header on a fresh file' {
        ...
        $first | Should -Match 'cluster-controller log opened'
    }
    ...
    Context 'reboot continuity (pointer file)' {
        ...
        It 're-attaches to the prior log file when a fresh pointer exists' {
            ...
            $line | Should -Match 'cluster-controller log resumed'
        }
    ...
    Context 'failure modes' {
        ...
        It 'throws an actionable error mentioning CLUSTERCTRL_LOG_DIR when default dir is not writable' {
            # Point CLUSTERCTRL_LOG_DIR at a path under a parent we will lock with NTFS deny ACL.
            ...
            try {
                $oldLogDir = $env:CLUSTERCTRL_LOG_DIR
                $env:CLUSTERCTRL_LOG_DIR = $readonly
                Reset-ClusterLogState
                { Initialize-ClusterLog } | Should -Throw -ExpectedMessage '*CLUSTERCTRL_LOG_DIR*'
            } finally {
                $env:CLUSTERCTRL_LOG_DIR = $oldLogDir
