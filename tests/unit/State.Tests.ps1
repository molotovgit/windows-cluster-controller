...
BeforeAll {
    ...
    $script:testRegBase = "HKCU:\Software\ClusterController-test-$([guid]::NewGuid().ToString('N').Substring(0,8))"
...
Describe 'Resume scheduled task' {
    ...
    Context 'New-ResumeTaskSpec (pure)' {
        ...
        It 'returns a spec with -Resume in the argument string and Trigger=Both by default' {
            ...
            $spec.TaskName | Should -Be 'ClusterControllerResume'
            $spec.TaskPath | Should -Be '\ClusterController\'
    ...
    Context 'Test-ResumeTask via the invoker seam' {
        ...
        It 'returns $true when invoker returns any non-null value' {
            & $script:State { Set-ResumeTaskInvoker -Operation Test -ScriptBlock { [pscustomobject]@{ TaskName = 'ClusterControllerResume' } } }
    ...
    Context 'Test seam gating' {
        ...
        It 'Set-ResumeTaskInvoker throws when CLUSTERCTRL_ALLOW_TEST_SEAMS is not set' {
            Remove-Item Env:CLUSTERCTRL_ALLOW_TEST_SEAMS -ErrorAction SilentlyContinue
            $thrown = $null
            try {
                & $script:State { Set-ResumeTaskInvoker -Operation Test -ScriptBlock { $null } }
    ...
    Context 'Get-ResumeTaskInfo' {
        It 'reports the canonical task name and dedicated folder' {
            ...
            $info.TaskName | Should -Be 'ClusterControllerResume'
            $info.TaskPath | Should -Be '\ClusterController\'
        }
    }
}
